...
const BulkInternalTransferForm = ({ transactions }: { transactions: UnreconciledTransaction[] }) => {
    ...
    const { call: createPaymentEntry, loading, error } = useFrappePostCall<{ message: { transaction: BankTransaction, payment_entry: PaymentEntry }[] }>('sge_bank_reconciliation.apis.bank_reconciliation.create_bulk_internal_transfer')
...
const InternalTransferForm = ({ selectedBankAccount, selectedTransaction }: { selectedBankAccount: SelectedBank, selectedTransaction: UnreconciledTransaction }) => {
    ...
    const { call: createPaymentEntry, loading, error, isCompleted } = useFrappePostCall<{ message: { transaction: BankTransaction, payment_entry: PaymentEntry } }>('sge_bank_reconciliation.apis.bank_reconciliation.create_internal_transfer')
    ...
    return <div className='grid grid-cols-4 gap-4'>
        {banks.map((bank) => (
            ...
            >
                {bank.logo ?
                    <img
                        src={`/assets/sge_bank_reconciliation/sge_bank_reconciliation/${bank.logo}`}
                        alt={bank.bank || ''}
...
const RecommendedTransferAccount = ({ transaction, onAccountChange }: { transaction: UnreconciledTransaction, onAccountChange: (account: string, is_mirror: boolean) => void }) => {
    ...
    const { data } = useFrappeGetCall('sge_bank_reconciliation.apis.bank_reconciliation.search_for_transfer_transaction', {
