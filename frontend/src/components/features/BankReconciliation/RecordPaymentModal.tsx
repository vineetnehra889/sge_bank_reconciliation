...
const BulkPaymentEntryForm = ({ transactions }: { transactions: UnreconciledTransaction[] }) => {
    ...
    const { call: createPaymentEntry, loading, error } = useFrappePostCall<{ message: { transaction: BankTransaction, payment_entry: PaymentEntry }[] }>('sge_bank_reconciliation.apis.bank_reconciliation.create_bulk_payment_entry_and_reconcile')
    ...
    const onPartyChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (event.target.value) {
            call.get('sge_bank_reconciliation.apis.bank_reconciliation.get_party_details', {
                company: company,
...
const PaymentEntryForm = ({ selectedTransaction, selectedBankAccount }: { selectedTransaction: UnreconciledTransaction, selectedBankAccount: SelectedBank }) => {
    ...
    const { call: createPaymentEntry, loading, error, isCompleted } = useFrappePostCall<{ message: { transaction: BankTransaction, payment_entry: PaymentEntry } }>('sge_bank_reconciliation.apis.bank_reconciliation.create_payment_entry_and_reconcile')
