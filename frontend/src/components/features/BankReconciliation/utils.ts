...
export const useGetAccountClosingBalanceAsPerStatement = (swrConfig: SWRConfiguration = {}) => {
    ...
    return useFrappeGetCall<{ message: { balance: number, date?: string } }>("sge_bank_reconciliation.apis.bank_account.get_closing_balance_as_per_statement", {
        bank_account: bankAccount?.name,
        date: dates.toDate
...
export const useGetUnreconciledTransactions = () => {
    const bankAccount = useAtomValue(selectedBankAccountAtom)
    const dates = useAtomValue(bankRecDateAtom)
    return useFrappeGetCall<{ message: UnreconciledTransaction[] }>('sge_bank_reconciliation.apis.transactions.get_bank_transactions', {
        bank_account: bankAccount?.name,
...
export const useGetBankTransactions = () => {
    const bankAccount = useAtomValue(selectedBankAccountAtom)
    const dates = useAtomValue(bankRecDateAtom)
    return useFrappeGetCall<{ message: BankTransaction[] }>('sge_bank_reconciliation.apis.transactions.get_bank_transactions', {
        bank_account: bankAccount?.name,
        from_date: dates.fromDate,
        to_date: dates.toDate,
...
export const useReconcileTransaction = () => {
    ...
    const { call, loading } = useFrappePostCall<{ message: BankTransaction }>('sge_bank_reconciliation.apis.bank_reconciliation.reconcile_vouchers')
...
export const useGetBankAccounts = (onSuccess?: (data?: Omit<SelectedBank, 'logo'>[]) => void, filterFn?: (bank: SelectedBank) => boolean) => {
    ...
    const { data, isLoading, error } = useFrappeGetCall<{ message: BankAccountWithCurrency[] }>('sge_bank_reconciliation.apis.bank_account.get_list', {
        company: company
    }, undefined, {
        revalidateOnFocus: false,
        revalidateIfStale: false,
