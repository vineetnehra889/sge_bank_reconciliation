...
const BulkBankEntryForm = ({ selectedTransactions }: { selectedTransactions: UnreconciledTransaction[] }) => {
    ...
    const { call, loading, error } = useFrappePostCall<{ message: { transaction: BankTransaction, journal_entry: JournalEntry }[] }>('sge_bank_reconciliation.apis.bank_reconciliation.create_bulk_bank_entry_and_reconcile')
...
const BankEntryForm = ({ selectedTransaction }: { selectedTransaction: UnreconciledTransaction }) => {
    ...
    const { call: createBankEntry, loading, error, isCompleted } = useFrappePostCall<{ message: { transaction: BankTransaction, journal_entry: JournalEntry } }>('sge_bank_reconciliation.apis.bank_reconciliation.create_bank_entry_and_reconcile')
    ...
    const onAccountChange = (value: string, index: number) => {
        ...
        if (value) {
            if (costCenterMapRef.current[value]) {
                ...
                call.get('sge_bank_reconciliation.apis.bank_reconciliation.get_account_defaults', {
                    account: value
                }).then((result: { message: string }) => {
